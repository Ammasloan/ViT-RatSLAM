...
public:
  ...
  void on_vpr_embedding(const std::vector<float> &embedding, int template_id, bool is_new, float match_score);
  bool uses_vpr_backend() const { return use_vpr_backend; }
...
private:
  ...
  int create_template();
  int create_template_from_embedding(const std::vector<float> &embedding);
  ...
  bool greyscale;
  bool use_vpr_backend;
  bool vt_log;
  int vt_log_separator_every;
  unsigned int vt_log_event_count;
  std::unordered_map<int, int> vpr_template_map;
